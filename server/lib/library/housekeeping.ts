...
import { getLibraryDir, isLibraryAvailable } from '../libraryPath.js';
 ...
 */
export function purgeJunkFiles(): { deleted: number; errors: number } {
  const LIBRARY_DIR = getLibraryDir();
  let deleted = 0;
  let errors = 0;
  if (!fs.existsSync(LIBRARY_DIR)) return { deleted, errors };

  for (const objectDir of fs.readdirSync(LIBRARY_DIR)) {
    const objPath = path.join(LIBRARY_DIR, objectDir);
    try {
      if (!fs.statSync(objPath).isDirectory()) continue;
      for (const fname of fs.readdirSync(objPath)) {
        if (!isRealFile(fname)) {
          try {
            fs.unlinkSync(path.join(objPath, fname));
            deleted++;
          } catch {
            errors++;
          }
...
export function scheduleAutoImport(): void {
  setInterval(() => {
    // Don't import while the library is being moved, or when its drive is not
    // reachable: a write would either fight the migration or recreate the path
    // on the wrong volume.
    if (isLibraryMigrating() || !isLibraryAvailable()) return;
