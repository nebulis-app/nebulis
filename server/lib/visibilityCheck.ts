 ...
 *
 * Server-side port of src/lib/visibilityCheck.ts (browser copy retained for
 * client-side chart rendering, which is display-only and not part of any
 * planning decision). Formulas must match exactly.
 *
...
import { SKY_MAP_AZ_SLICES, SKY_MAP_BANDS, SKY_MAP_CELLS } from './skyMapConfig.js';

export { SKY_MAP_AZ_SLICES, SKY_MAP_BANDS, SKY_MAP_CELLS };
 ...
 * sky map. Samples at `stepMinutes` cadence (5 minutes by default).
 */
