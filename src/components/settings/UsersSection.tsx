...
import { Key, Plus, ShieldCheck, Eye } from 'lucide-react';
import { getUsers, createUser, deleteAppUser, resetUserPassword, updateUserRole, toUserRole, type UserRole } from '../../lib/api/auth';
        ...
        {users.length === 0 ? (
          ...
          <div className="space-y-1.5">
            {users.map(user => (
              <div
                key={user.id}
                className={`flex items-center gap-4 p-3.5 rounded-xl transition-colors duration-150 ${
                ...
                {resetPwUserId === user.id ? (
                  ...
                  <div className="flex items-center gap-1">
                    <select
                      ...
                      className={`text-xs px-2 py-1.5 rounded-lg border appearance-none cursor-pointer transition-all duration-150 ${
                        isDark
